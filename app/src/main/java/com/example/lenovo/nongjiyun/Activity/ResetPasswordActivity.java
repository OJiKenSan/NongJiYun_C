...
public class ResetPasswordActivity extends AppCompatActivity {
    ...
    private void nextStep(){
        btn_phone_reset_password_next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (tag=true){
                    ToastMessage.initToastMessage("重设密码成功",ResetPasswordActivity.this);
    ...
    private Boolean initFileter(String password,String commitpassword) {
        if (password.length()==0){
            ToastMessage.initToastMessage("请输入密码",ResetPasswordActivity.this);
        }else {
            if (password.equals(commitpassword)){
                return true;
            }else {
                ToastMessage.initToastMessage("密码不一致，请重新输入",ResetPasswordActivity.this);
            }
        }
